  ...
  const fetchFeaturedProducts = async () => {
    try {
      const { data, error } = await supabase
        .from('products')
        .select('id, name, description, price, image_url, stock, details, nutritional_info')
        .limit(3);
      ...
      if (data) {
        ...
        // Transform data to add ratings, discounts, and Cloudinary image URLs
        const productsWithRatings = data.map(product => {
          // Define an array of Cloudinary image URLs for makhana products
          ...
          // Assign a random Cloudinary image URL to each product
          const randomImageUrl = cloudinaryImages[Math.floor(Math.random() * cloudinaryImages.length)];
          
          return {
            ...product,
            image_url: randomImageUrl, // Override the image_url with Cloudinary URL
              ...
              {featuredProducts.map((product, index) => (
                ...
                >
                  <div className="aspect-square overflow-hidden relative">
                    <img 
                      src={product.image_url || 'https://res.cloudinary.com/dlvxjnycr/image/upload/v1745910077/4_rrgvey.jpg'}
                      alt={`${product.name} - Premium Makhana`}
                      className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
                      onError={(e) => {
                        const target = e.target as HTMLImageElement;
                        target.src = 'https://res.cloudinary.com/dlvxjnycr/image/upload/v1745910077/4_rrgvey.jpg';
                      }}
                    />
